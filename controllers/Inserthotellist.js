...
const Inserthotel_List = async (req, res) => {
    ...
    try {
        ...
        const hotelImageQuery = `
            INSERT INTO hotel_images (hotel_id, image_url, image_description,image) VALUES (?, ?, ?,?)
        `;
        for (const image of hotel_images) {
            await connection.query(hotelImageQuery, [
                hotelId, image.image_url, image.image_description,image.image
            ]);
        }
...
*/
// controllers/Inserthotellist.js
...
const Inserthotel_List = async (req, res) => {
    const {
        ...
        meals = [], // Initialize meals as an empty array if not provided
        hotel_images = [], // Initialize hotel_images as an empty array if not provided
        front_images = [], // Initialize front_images as an empty array if not provided
        latitude,
        longitude
    } = req.body;
    ...
    try {
        ...
        const insertHotelQuery = `
            INSERT INTO hotel_list 
                (name, total_no_of_guests, total_no_of_rooms, location, price_per_room, about, policy_rules, facilities, FAQ, locationcodeaddress, home_rules_and_truths, amenities)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        const [insertHotelResult] = await connection.query(insertHotelQuery, [
            name,
        ...
        for (const image of hotel_images) {
            await connection.query(hotelImageQuery, [
                hotelId, image.image_url, JSON.stringify(image.image_description), image.image
            ]);
        }
        ...
        for (const frontImage of front_images) {
            await connection.query(frontImageQuery, [
                hotelId, frontImage.image_data, frontImage.image_description
            ]);
        }
